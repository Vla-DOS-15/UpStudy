...
import { Archive, LayoutList, List, Loader2 } from 'lucide-react';
...
export default function ArchivedOrdersClient() {
    ...
    const fetchArchivedOrders = async (pageNum: number, isLoadMore = false) => {
        try {
            if (isLoadMore) {
            ...
            const data = await orderService.getArchivedOrders({ page: pageNum, pageSize: PAGE_SIZE });
    ...
    useEffect(() => {
        setPage(1);
        fetchArchivedOrders(1, false);
    }, [viewMode]);

    useEffect(() => {
        if (page > 1) {
            fetchArchivedOrders(page, viewMode === 'infinite');
        }
    }, [page]);
