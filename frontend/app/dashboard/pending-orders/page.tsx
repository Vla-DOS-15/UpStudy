...
import { Clock, FileText, Download, Loader2, XCircle } from 'lucide-react';
...
export default function PendingOrdersPage() {
    ...
    const fetchOrders = async () => {
        try {
            ...
            // Filter for Pending
            setOrders(data.filter((o: any) => o.myProposalStatus === 'Pending'));
        } catch (error) {
            console.error(error);
            toast.error('Не вдалося завантажити замовлення в очікуванні');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleCancelProposal = async (proposalId: string) => {
        try {
            ...
            toast.success('Ставку скасовано');
            // Remove from list
            setOrders(prev => prev.filter(o => o.myProposalId !== proposalId));
        } catch (error) {
            console.error(error);
            toast.error('Не вдалося скасувати ставку');
        }
    };
    ...
    const EmptyState = () => (
        <div className="text-center py-12 border-2 border-dashed rounded-xl bg-muted/20">
            ...
            <p className="text-muted-foreground font-medium">Немає замовлень в очікуванні</p>
        </div>
    );
    ...
    return (
        <div className="space-y-6 container max-w-5xl mx-auto py-6">
            <div className="flex items-center justify-between">
                <h2 className="text-3xl font-bold tracking-tight">В очікуванні</h2>
            </div>
                        ...
                        <Card key={order.id} className="hover:shadow-md transition-shadow">
                            <CardHeader className="pb-3">
                                <div className="flex justify-between items-start gap-4">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <span className="text-lg font-bold text-green-600">{order.myPrice} ₴</span>
                                    </div>
                                </div>
                            </CardHeader>
                            <CardContent>
                            ...
                            <CardFooter className="bg-muted/5 flex justify-end p-4 rounded-b-xl border-t">
                                <Button
                                    ...
                                    onClick={() => handleCancelProposal(order.myProposalId)}
                                >
